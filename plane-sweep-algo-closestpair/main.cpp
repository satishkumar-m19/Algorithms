...
class CTree
{
    public:
        ...
        void inorder(ctree, Point, int &, Point &, Point &, int &);
        bool find(ctree, int);
        CTree()
        {}
};
 ...
 */
void CTree::inorder(ctree ptr,Point P,int& d , Point& closePairP1,Point& closePairP2, int& maxTempNodes)
{
    maxTempNodes++;
    int d_temp;
    if (ptr == nullnode)
    ...
    if (ptr != NULL)
    {
        inorder(ptr->left, P, d , closePairP1, closePairP2, maxTempNodes);
        ...
        inorder(ptr->right, P, d , closePairP1, closePairP2, maxTempNodes);
    }
}
...
void processPoints(int n, Point P[], CTree c){
    ...
    int d=0, l=0, r=2, bstLength=0, maxNodes=0, maxTempNodes=0;
    ...
    while(r<=n){
        //for(int j=0;j<;j++){
            //cout<<" Here ";
            maxTempNodes=0;
            cout<< "max node initialisations : " << r << "    " << maxTempNodes << endl;
            c.inorder(root, nextPoint, d , closePairP1, closePairP2, maxTempNodes); // for is maintained here
            cout<< "max node after computation : " << r << "    " << maxTempNodes << endl;
            if(maxTempNodes>maxNodes){
                maxNodes = maxTempNodes;
            }
    ...
    cout << "Bst Nodes : " << bstLength <<endl;
    cout << "Bst Nodes : " << maxNodes <<endl;
    cout << "Bst Nodes : " << maxTempNodes <<endl;
}
